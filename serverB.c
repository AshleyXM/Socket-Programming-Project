...
int main(){
	...
	while(fgets(temp, 2000, fp) != NULL){ // read by line
		...
		for(int i = 0; i < MAXSLOTLEN; i++){
		   printf("%d ", users[usernum].slots[i]);
		}
		printf("\n");
		*/
	...
	while(1){
      ...
      printf("Server B received the usernames from Main Server using UDP over port %d.\n", MYPORT);
	   ...
	   if(strcmp(selected, "[]") != 0) {
	      ...
	      printf("ServerB finished updating.\n");
		}
...
void removeInterval(char *recvnames, char *selected) {
   ...
   for(int i = 0; i < num; i++) {
      for(int j = 0; j < usernum; j++) { // iterate all the users in serverB
